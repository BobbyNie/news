...
按 `prompts/cursor-stock-daily.md` 执行；须含 **港股和美股近期 IPO 专栏** 与 **事件驱动股票专题**。不得在合并 prompt 中硬编码固定股票、固定人物或固定公司作为每日必写对象；股票关注必须由当天可核验的公开披露、交易所文件、公司 IR、财报、重大监管/诉讼、异常成交或主流市场媒体共同触发。
...
| 语音朗读 | `reader-controls` + `../../assets/report-reader.js` 共用 JS；`reader-settings-toggle` 打开设置窗；只朗读正文；可选 Google TTS key；普通话 | 同上 |
| 页宽 | 760px | 780px |
